...
/**
 * Carga la configuración externa desde data/config.json.
 * Si falla, usa valores por defecto para no bloquear el flujo.
 */
...
async function getAppConfig() {
  ...
  try {
    ...
    if (!response.ok) throw new Error("No se pudo cargar data/config.json");
    const config = await response.json();
    const precioKw = Number(config.precio_kw);

    appConfigCache = {
      precio_kw: Number.isFinite(precioKw) && precioKw > 0 ? precioKw : 1000,
    };
  } catch (error) {
    console.warn("[ParquesSJ] No fue posible leer data/config.json, se usará configuración por defecto.", error);
    appConfigCache = { precio_kw: 1000 };
  }

  return appConfigCache;
}

function calcularKwRecarga(montoCOP, precioKw) {
  return montoCOP / precioKw;
}

function mostrarVistaConfirmacion({ amountCOP, kwTotal, transactionRef }) {
  successPaidAmount.textContent = formatCOP(amountCOP);
  successKw.textContent = kwTotal.toFixed(2);
  successRef.textContent = transactionRef || "N/A";
...
const successPaidAmount = document.getElementById("successPaidAmount");
const successKw = document.getElementById("successKw");
const successRef = document.getElementById("successRef");
const btnNewCharge = document.getElementById("btnNewCharge");
...
btnNewCharge.addEventListener("click", () => {
  mostrarVistaRecarga();
  chargeForm.reset();
  resultArea.innerHTML = "";
  amountHint.innerHTML = "Ingresa un valor mínimo de <strong>$2.000 COP</strong>";
...
chargeForm.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    // Cálculo preliminar de energía a cargar con el valor externo de precio_kw
    const kwEstimados = calcularKwRecarga(monto, config.precio_kw);
    ...
    checkout.open(function (result) {
      ...
      if (status === "APPROVED" || status === "approved" || status === "APROBADA" || status === "aprobada") {
        const amountPaidCOP = Number.isFinite(transaction.amount_in_cents) ? transaction.amount_in_cents / 100 : monto;

        const kwTotal = Number.isFinite(amountPaidCOP)
          ? calcularKwRecarga(amountPaidCOP, config.precio_kw)
          : kwEstimados;

        const transactionRef = transaction.reference || transaction.id || referencia;

        // Se reemplaza la vista de recarga por la tarjeta de éxito.
        mostrarVistaConfirmacion({
          amountCOP: amountPaidCOP,
          kwTotal,
          transactionRef,
        });
      } else {
